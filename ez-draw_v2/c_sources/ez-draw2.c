...
Ez_window ez_window_createex (int x, int y,int w, int h, const char *name, Ez_func func)
{
    Ez_window win;
    Ez_win_info *info;
...
{
    if((ezx.icolor != 0) && (ezx.color == color)) 
		return;
	ezx.color = color;
	ezx.icolor = 1;
...
{
    if((ezx.ithick != 0) && (ezx.thick == thick)) 
		return;
...
{
#ifdef EZ_BASE_XLIB
	if(ezx.pixcolor != color)
	{
		XSetForeground (ezx.display, ezx.gc, color);
		ezx.pixcolor = color;
	}		
