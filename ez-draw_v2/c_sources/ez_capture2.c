...
//char *GpPixels = NULL;
    ...
    HBITMAP old_hbitmap = (HBITMAP)SelectObject(CaptureDC, CaptureBitmap);
    BitBlt(CaptureDC, 0, 0, Width, Height, DevC, 0, 0, SRCCOPY|CAPTUREBLT);

    //BITMAP bmp;
//    GetObject(CaptureBitmap, sizeof(BITMAP), &bmp);
    ...
    *GWidth = Width ;
    *GHeight = Height ;
    DWORD dwBmpSize = Width  * Height  * 3;

	//HANDLE hDIB = GlobalAlloc(GHND, dwBmpSize);
	//char *pPixels = (char *)GlobalLock(hDIB);
    Ez_uint8 *pPixels = malloc(dwBmpSize);
    GetDIBits(CaptureDC, CaptureBitmap, 0, Height, pPixels, (BITMAPINFO *)&bi, DIB_RGB_COLORS);

    //GpPixels = malloc(dwBmpSize);
    //CopyMemory(GpPixels, pPixels, dwBmpSize);

	//GlobalUnlock(hDIB);
	//GlobalFree(hDIB);
	
	//SelectObject(CaptureDC,old_hbitmap);
